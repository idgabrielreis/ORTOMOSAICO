/** @type {import('next').NextConfig} */
const apiTarget = process.env.API_URL ?? "http://localhost:8000";

// Com STATIC_EXPORT=1 o frontend vira arquivos estáticos, servidos pelo próprio
// backend. É o que permite empacotar tudo em um executável, sem Node na máquina
// do usuário. Em desenvolvimento continua sendo servidor Next com proxy da API.
const isStaticExport = process.env.STATIC_EXPORT === "1";

const nextConfig = {
  reactStrictMode: true,
  // O progresso do processamento chega por Server-Sent Events. Com a compressão
  // do Next ligada, os eventos ficam presos no buffer do gzip e a tela só
  // atualiza no fim: a barra congela e tempo, CPU e RAM aparecem vazios.
  compress: false,
  ...(isStaticExport
    ? { output: "export", images: { unoptimized: true }, trailingSlash: true }
    : {
        async rewrites() {
          return [{ source: "/api/:path*", destination: `${apiTarget}/api/:path*` }];
        },
      }),
};

export default nextConfig;
